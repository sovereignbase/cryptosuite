...
function generateParamsOf(a: Algorithms): AlgorithmIdentifier {
...
export async function generateDigitalSignatureKeypair(
  ...
  const signJwk = normalizeSignJWK({ ...rawSign, alg, use: 'sig' })
  const verifyJwk = normalizeVerifyJWK({ ...rawVerify, alg, use: 'sig' })

  return { signJwk, verifyJwk }
}

export const generateDigitalSignatureCryptoKeyPair =
  generateDigitalSignatureKeypair
export const generateVerificationPair = generateDigitalSignatureKeypair
